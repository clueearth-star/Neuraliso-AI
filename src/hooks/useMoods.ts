import { useState, useEffect, useCallback } from "react";
import { MoodEntry } from "../types";
import { useAuth } from "../contexts/AuthContext";
import { supabase, supabaseRetry, isSupabaseConfigured } from "../lib/supabase";
import { storage } from "../lib/storage";

export function useMoods() {
  const { user, session, loading: authLoading } = useAuth();
  const [data, setData] = useState<MoodEntry[] | null>(null);
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<Error | null>(null);

  const fetchMoods = useCallback(async () => {
    if (authLoading) return;

    setLoading(true);
    setError(null);

    // Priority 1: Try Supabase if authenticated & configured
    if (user && isSupabaseConfigured() && !storage.get("neuraliso_is_anonymous")) {
      const { data: remoteData, error: remoteErr } = await supabaseRetry(async () => {
        return await supabase
          .from("moods")
          .select("*")
          .eq("user_id", user.id)
          .order("date", { ascending: false });
      });

      if (!remoteErr && remoteData) {
        const mapped: MoodEntry[] = remoteData.map((m: any) => ({
          id: m.id,
          date: m.date,
          timestamp: new Date(m.created_at || m.date).getTime() || Date.now(),
          score: m.score,
          emoji: m.emoji || "😐",
          label: m.label || "Neutral",
          note: m.note || "",
          tags: m.tags || [],
        }));
        storage.set("neuraliso_moods_v2", JSON.stringify(mapped));
        setData(mapped);
        setLoading(false);
        return;
      }
    }

    // Priority 2 & 3: Fallback to localStorage or empty array
    try {
      const local = storage.getMoods();
      setData(local);
    } catch (err: any) {
      setError(err);
      setData([]);
    } finally {
      setLoading(false);
    }
  }, [user, authLoading]);

  useEffect(() => {
    fetchMoods();
    const handleStorageUpdate = () => {
      setData(storage.getMoods());
    };
    if (typeof window !== "undefined") {
      window.addEventListener("neuraliso-storage-updated", handleStorageUpdate);
      return () => window.removeEventListener("neuraliso-storage-updated", handleStorageUpdate);
    }
  }, [fetchMoods]);

  return { data: authLoading ? null : data, loading: authLoading || loading, error, refetch: fetchMoods };
}

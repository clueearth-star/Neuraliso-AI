...
export const AnalyticsView: React.FC<AnalyticsProps> = ({ 
  ...
  return (
    <div id="analytics-view-container" className="pb-24 space-y-6 max-w-xl mx-auto px-1 animate-fade-in">
      ...
      {/* PATTERN INSIGHTS / DEEPER ANALYTICS: LOCKED PREVIEW IF TRIAL (Requirement 2) */}
      <div className="relative overflow-hidden rounded-2xl">
        {!canAccessInsights && (
          <div className="absolute inset-0 z-20 backdrop-blur-md bg-white/75 flex flex-col items-center justify-center p-6 text-center border border-amber-200/80 rounded-2xl shadow-lg">
            <div className="w-12 h-12 bg-amber-100 rounded-full flex items-center justify-center text-amber-700 mb-3 shadow-inner">
              <Lock className="w-6 h-6" />
            </div>
            <span className="text-[10px] font-bold uppercase tracking-wider text-amber-800 bg-amber-100 px-3 py-1 rounded-full mb-2">
              Premium Feature
            </span>
            <h3 className="font-serif italic font-bold text-xl text-dark-text">Deep Pattern Insights Locked</h3>
            <p className="text-xs text-muted-text max-w-sm mt-1.5 leading-relaxed">
              Deeper analytics, biological frequency correlations, and multi-week trend forecasting are exclusive to Premium subscribers.
            </p>
            <button
              onClick={onOpenUpgrade}
              className="mt-4 bg-deep-sage text-white text-xs font-bold py-2.5 px-6 rounded-full shadow-md hover:bg-primary-sage transition-all flex items-center gap-2 cursor-pointer active:scale-95"
            >
        ...
        <div className={!canAccessInsights ? "filter blur-xs pointer-events-none select-none opacity-60" : ""}>
          {/* 1. MOOD TRENDS LINE GRAPH */}
          <div className="wellness-card p-5 border bg-white space-y-3 mb-6">
            <div className="flex justify-between items-center">
              ...
              <span className="text-[10px] font-mono text-muted-text">TRENDS & CORRELATIONS</span>
            </div>
      ...
      {/* 3. HISTORY TIMELINE LOGS (Limit notice if in trial) */}
      <div className="wellness-card p-5 border bg-white space-y-3">
        ...
        {isTrial && (
          <div className="p-2.5 bg-amber-50/80 border border-amber-200/60 rounded-xl text-[11px] text-amber-900 flex items-center justify-between">
            <span>🗓️ Free trial displays your entries from the last 7 days. Upgrade to Premium for unlimited historical archives!</span>
            {onOpenUpgrade && (
              <button 
                onClick={onOpenUpgrade}
                className="text-[10px] font-bold text-amber-800 underline hover:text-amber-950 ml-2 whitespace-nowrap"
              >
                Upgrade
              </button>
            )}
